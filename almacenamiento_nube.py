...
def administrar_instancia_ec2(eliminar=False):
    ...
    instance_region = instance_description["Reservations"][0]["Instances"][0][
        "Placement"
    ]["AvailabilityZone"][:-1]
...
# volumen_id = crear_y_asignar_volumen_ebs(instance_id=os.getenv("INSTANCE_ID"), zona_disponibilidad='c')
...
def montar_volumen_ebs_en_instancia(
    ...
    ssh.connect(hostname=instance_ip, username=username, pkey=key, port=22, timeout=10)
...
montar_volumen_ebs_en_instancia(instance_ip=os.getenv("INSTANCE_IP"))
